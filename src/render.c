...
{
    *state->buttons[SAND] = (button_t){ .selected = (state->selected_material == SAND), .rect = (SDL_Rect){state->window_rect.w - 48, 0, 48, 48 } };
    *state->buttons[WATER] = (button_t){ .selected = (state->selected_material == WATER), .rect = (SDL_Rect){state->window_rect.w - 48, 48, 48, 48 } };
    *state->buttons[ACID] = (button_t){ .selected = (state->selected_material == ACID), .rect = (SDL_Rect){state->window_rect.w - 48, 96, 48, 48 } };
    *state->buttons[VIRUS] = (button_t){ .selected = (state->selected_material == VIRUS), .rect = (SDL_Rect){state->window_rect.w - 48, 144, 48, 48 } };
    *state->buttons[WOOD] = (button_t){ .selected = (state->selected_material == WOOD), .rect = (SDL_Rect){state->window_rect.w - 48, 192, 48, 48 } };
    *state->buttons[METAL] = (button_t){ .selected = (state->selected_material == METAL), .rect = (SDL_Rect){state->window_rect.w - 48, 240, 48, 48 } };
    *state->buttons[ERASER] = (button_t){ .selected = (state->selected_material == ERASER), .rect = (SDL_Rect){state->window_rect.w - 48, state->window_rect.h - 96, 48, 48 } };
    *state->buttons[RESET] = (button_t){ .selected = 0, .rect = (SDL_Rect){ state->window_rect.w - 48, state->window_rect.h - 48, 48, 48 } };
    ...
    render_text(state, material_text[state->selected_material], 10, state->window_rect.h - 30, white);
